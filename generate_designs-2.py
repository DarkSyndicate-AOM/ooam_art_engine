...
def generate_batch(niche_key: str, count: int = BATCH_SIZE) -> list:
...
{{
  ...
  "image_prompt": "Detailed FLUX/image prompt (80-120 words) describing the art",
...
def export_csv(library: list):
    ...
    log.info(f"  Exported {len(rows)} rows to {LISTINGS_FILE}")
...
def main():
    ...
    parser.add_argument("--loop",   action="store_true", help="Run on schedule every 6h")
