...
Why FLUX1.1 Pro via fal.ai:
  ...
  - ~$0.04-0.06 per image at 3000x4000px print resolution
  - No subscription — pure pay-per-use
  - Commercial license: images are yours to sell
...
# FLUX1.1 Pro on fal.ai
FAL_MODEL = "fal-ai/flux-pro/v1.1"

# Print-ready resolution for 16x20" canvas at 150-200 DPI
IMAGE_WIDTH  = 3000
IMAGE_HEIGHT = 4000

# Cost guard: stop if batch would exceed this
...
def upload_image_to_printify(image_path: Path, filename: str, shop_id: str) -> str:
    ...
    payload = {"file_name": filename, "contents": data}
...
def enhance_prompt_for_print(raw_prompt: str, style: str, palette: list) -> str:
    style_suffix = {
...
def generate_image_flux(prompt: str, design_id: str) -> Path:
    fal = _get_fal()
    os.environ["FAL_KEY"] = FAL_KEY or ""

    output_path = IMAGES_DIR / f"{design_id}.png"
    if output_path.exists():
        log.info(f"  Skipping (already exists): {output_path.name}")
        return output_path
...
def render_pending(limit=None, target_id=None, dry_run=False):
    ...
    if estimated_cost > MAX_SPEND_PER_RUN_USD:
        log.warning(
            ...
            f"${MAX_SPEND_PER_RUN_USD:.2f}. Use --limit N to reduce batch."
        )
        return
    ...
    for design in queue:
        ...
        if image_path and shop_id:
            ...
            if design.get("printify_id") and design["printify_id"] != "placeholder":
                try:
                    patch_product_image(design["printify_id"], printify_image_id, shop_id)
    ...
    log.info(f"Estimated spend: ${rendered * COST_PER_IMAGE_USD:.2f}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--limit",     type=int, help="Max images to render")
    parser.add_argument("--design-id", type=str, help="Render one design by ID")
    parser.add_argument("--dry-run",   action="store_true",
                        help="Preview prompts without calling fal.ai")
