...
BLUEPRINT_MAP = {
    "canvas art":          {"blueprint_id": 1209, "print_provider_id": 99},
    "framed poster":       {"blueprint_id": 1188, "print_provider_id": 99},
    "metal print":         {"blueprint_id": 1146, "print_provider_id": 99},
    "black & white print": {"blueprint_id": 1188, "print_provider_id": 99},
}
...
def build_product_payload(design: dict, shop_id: str) -> dict:
        ...
        for v in all_variants:
            if label in v.get("title", ""):
                selected_variants.append({
                    "id":         v["id"],
                    "price":      int(design.get("price", 49.99) * 100),
                    "is_enabled": True,
                })
                break
    ...
    payload = {
                ...
                "placeholders": [
                    {
                        ...
                        "images": [
                            {
                                "id":    design.get("printify_image_id", "placeholder"),
                                "x":     0.5,
                                "y":     0.5,
                                "scale": 1,
                                "angle": 0,
        ...
        "tags": design.get("amazon_keywords", [])[:13],
    }
    return payload
...
def upload_pending(dry_run: bool = False):
    ...
    pending  = [d for d in library if d.get("status") in ("pending", "image_ready")]
    shop_id  = get_shop_id()
